...
#define lua_tonumberx(L, i, b) \
  (lua_isnumber(L, (i)) ? (*(b) = 1, lua_tonumber(L, (i))) : (*(b) = 0, 0))
#define lua_tointegerx(L, i, b) \
  (lua_isnumber(L, (i)) ? (*(b) = 1, lua_tointeger(L, (i))) : (*(b) = 0, 0))
...
static inline void luaX52_luaL_setfuncs(lua_State *L, const luaL_Reg *l,
                                        int nup) {
...
static inline int luaX52_lua_compare(lua_State *L, int index1, int index2,
                                     int op) {
  ...
  switch (op) {
    ...
    default:
      return luaL_error(L, "Call to lua_compare with unsupported operator %d",
                        op);
  }
}

static inline int luaX52_luaL_getsubtable(lua_State *L, int idx,
                                          const char *fname) {
...
static inline void luaX52_luaL_requiref(lua_State *L, const char *modname,
...
static inline void *luaX52_luaL_testudata(lua_State *L, int ud,
                                          const char *tname) {
