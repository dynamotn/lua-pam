...
luaXt_declmethod(s, void, meth, 2, int, x, int,
                 y);  // FIXME What is this supposed to produce?
...
#define luaXt_declcfunction(function, ...)               \
  luaXt_declcfunction_impl(function, STRINGIFY function, \
                           NUM_ARGS(__VA_ARGS__) / 2,    \
...
void luaXt_declcfunction_impl(lua_CFunction function, const char *fname,
                              int nargs, ...) {
  va_list ap;

  luaXt_CFunction *f =
      malloc(sizeof(luaXt_CFunction) + nargs * sizeof(luaXt_Argument));
  *f = (luaXt_CFunction){function, strdup(fname), nargs};
...
int wrap_cmethod(lua_State *L) {
  ...
  void *self = luaX_testclass(L, 1, m->self->name,
                              "self");  // FIXME Why not use checkclass instead?
  if (self == NULL) {
  ...
  for (int i = 0; i < m->f.nargs; i++) {
    ...
    if (!arg->type->is(L,
                       i + 1)) {  // FIXME Why not use check functions instead?
...
#define luaXt_declfunction(rtype, function, ...)                         \
  luaXt_declfunction_impl(STRINGIFY rtype, function, STRINGIFY function, \
...
void luaXt_declfunction_impl(const char *rtype, lua_CFunction function,
                             const char *fname, int nargs, ...) {}
