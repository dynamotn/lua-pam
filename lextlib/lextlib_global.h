 ...
 *
 *  OS and CC detection code shamelessly stolen from Qt4 (Qt/qglobal.h) by
 * Dennis. This has been stripped down, feel free to add checks as you need
 * them.
 */
...
#define LUAX_OS_MAC  /* LUAX_OS_MAC is mostly for compatibility, but also more \
                        clear */
...
#else
#error \
    "Warzone has not been tested on this compiler. Please contact warzone-dev@gna.org"
#endif /* LUAX_CC_x */
...
#else
#error \
    "Warzone has not been tested on this window system. Please contact warzone-dev@gna.org"
#endif /* LUAX_WS_x */
...
#define LUAX_CC_INTEL_PREREQ(maj, min) ((__ICC) >= ((maj)*100) + (min))
#else
 ...
 * the LUAX_DECL_DEPRECATED macro is inserted at the front, right before the
 ...
 * GCC: "The format attribute specifies that a function takes printf, scanf,
 * strftime or strfmon style arguments which should be type-checked against a
 * format string."
 */
...
/*!
 * \def LUAX_DECL_CONST
 * GCC: "Many functions do not examine any values except their arguments, and
 * have no effects except the return value. Basically this is just slightly more
 * strict class than the pure attribute below, since function is not allowed to
 * read global memory."
 */
#if LUAX_CC_GNU_PREREQ(2, 5) && !defined(LUAX_CC_INTEL)
...
/*!
 * \def LUAX_DECL_PURE
 * GCC: "Many functions have no effects except the return value and their return
 * value depends only on the parameters and/or global variables. Such a function
 * can be subject to common subexpression elimination and loop optimization just
 * as an arithmetic operator would be."
 */
 ...
 * GCC: "This attribute, attached to a function, means that the function is
 ...
 * GCC: "The warn_unused_result attribute causes a warning to be emitted if a
 ...
 * GCC: "Accesses to objects with types with this attribute are not subjected to
 * type-based alias analysis, but are instead assumed to be able to alias any
 * other type of objects, just like the char type. See -fstrict-aliasing for
 * more information on aliasing issues."
 */
 ...
 * The compiler may assume that the memory referenced by a "restrict" pointer is
 * not aliased by any other pointer. Thus this forms the opposite of
 * LUAX_DECL_MAY_ALIAS.
 */
 ...
 * Declares a variable to be local to the running thread, and not shared between
 * threads.
 */
#if defined(LUAX_CC_GNU) || defined(LUAX_CC_INTEL)
...
#define LUAX_ASSERT_STATIC_STRING(_var) \
  assert(__builtin_types_compatible_p(typeof(_var), char[]))
#else
...
#pragma warning( \
    disable : 4100) /* Shut up: unreferenced formal parameter (FIXME) */
#pragma warning(disable : 4127) /* Shut up: conditional expression is constant \
                                   (eg. "while(0)") */
#pragma warning( \
    disable : 4204) /* Shut up: non-constant aggregate initializer */
#pragma warning(disable : 4244) /* Shut up: conversion from 'float' to 'int', \
                                   possible loss of data */
#pragma warning(disable : 4267) /* Shut up: conversion from 'size_t' to \
