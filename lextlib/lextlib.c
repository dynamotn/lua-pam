...
int luaX_panic(lua_State *L) {
  ...
  if (nresults != 1) {
    fprintf(stderr,
            "Unable to get backtrace: Unexpected number of results (%d)\n",
            nresults);
    return 0;
  }
  ...
  if (err == NULL) {
    fprintf(stderr,
            "Unable to get backtrace: Cannot convert result to string\n");
    return 0;
  }
...
void luaX_showstack(lua_State *L) {     //> [-0,+0,e]
  ...
  if (lua_pcall(L, 1, 1, 0) != LUA_OK) {
    fprintf(stderr,
            "Unable to get backtrace: Executing traceback function failed\n");
    return;
  }
...
void luaX_preload(lua_State *L, const char *name, lua_CFunction init_function) {
  ...
  lua_setfield(L, -2,
               name);  // package.preload[name] = init_function  // [-1,+0,e]
  lua_pop(
      L,
      2);  // pop (package.preload, package)                     // [-2,+0,-]
}
...
void luaX_restrict(lua_State *L) {
  ...
  lua_newtable(
      L);  // push ($newtable)                                // [-0,+1,e]
  lua_rawgeti(L, -2,
              1);  // push (package.searchers[1])               // [-0,+1,-]
  lua_rawseti(L, -2,
              1);  // $newtable[1] = top, pop                   // [-1,+0,e]
  lua_setfield(L, -3,
               "searchers");  // package.searchers = top, pop   // [-1,+0,e]
  lua_pop(L,
          2);  // pop ($package.searchers, package)                 // [-2,+0,-]
}
...
int luaX_argerror(lua_State *L, int narg, const char *argname,
                  const char *extramsg) {
  const char *msg = NULL;
...
static int typeerror(lua_State *L, int narg, const char *argname,
                     const char *tname) {
  const char *msg =
      lua_pushfstring(L, "%s expected, got %s", tname, luaX_typename(L, narg));
...
void luaX_checktype(lua_State *L, int narg, const char *argname, int t) {
  if (lua_type(L, narg) != t) tag_error(L, narg, argname, t);
}
...
lua_Number luaX_optnumber(lua_State *L, int narg, const char *argname,
                          lua_Number def) {
...
lua_Integer luaX_optinteger(lua_State *L, int narg, const char *argname,
                            lua_Integer def) {
  return luaX_opt(L, luaX_checkinteger, narg, argname, def);
}

const char *luaX_checklstring(lua_State *L, int narg, const char *argname,
                              size_t *len) {
...
const char *luaX_optlstring(lua_State *L, int narg, const char *argname,
...
void *luaX_checkudata(lua_State *L, int narg, const char *argname,
                      const char *tname) {
...
void *luaX_optudata(lua_State *L, int narg, const char *argname,
                    const char *tname, void *def) {
  if (lua_isnoneornil(L, narg)) {
    return def;
  }
...
void *luaX_checkclass(lua_State *L, int narg, const char *cname,
                      const char *argname) {
